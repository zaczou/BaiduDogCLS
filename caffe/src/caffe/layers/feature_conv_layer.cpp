...
void FeatureConvolutionLayer<Dtype>::compute_output_shape() {
...
template <typename Dtype>
void FeatureConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void FeatureConvolutionLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
	const vector<Blob<Dtype>*>& top) {
	const int first_spatial_axis = this->channel_axis_ + 1;
...
template <typename Dtype>
void FeatureConvolutionLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  int weight_size = bottom[1]->count(1);
    ...
    for (int n = 0; n < this->num_; ++n){
		//conventional convoltion 		
		this->forward_cpu_gemm(bottom_data + n * this->bottom_dim_, weight + n * weight_size,top_data + n * this->top_dim_);
...
template <typename Dtype>
void FeatureConvolutionLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(FeatureConvolutionLayer);
#endif

INSTANTIATE_CLASS(FeatureConvolutionLayer);
REGISTER_LAYER_CLASS(FeatureConvolution);
